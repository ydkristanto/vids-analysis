...
```         
curl \
  'https://youtube.googleapis.com/youtube/v3/videos?part=contentDetails%2Csnippet&id=IYSIzMNvT0k%2ChcUC4IFisL4%2Co7p5-2gqPc4%2Ck_krAa5BcUc%2C2Ftvle6k9as%2CriRa1fv7XAo%2CfS8JmmjCg4Y%2CBSlzOjW5gsQ%2CLSsfEfKJPUA%2C7Ji0rZA-D7k%2CBZsxf1LWa38%2CKx-urubrfl4%2CTeJ_Dht6N4E%2CquoIHoqCJs4%2CyA9kwmITnkk%2CzG5DYRKRrRI%2Ch8FZ_H_zIlA%2COXRHUDd28uM%2Cs-HX0NcVGco%2CeJizsGLUFFw%2CwjQracmCEag%2Cr5Xkh25jgwY%2COiE3M7m9dxs%2CdPNrkm12e5M%2CWnE0IIO0EQ0%2CwMzpIezHdcs%2CIaUXOCf73Lg%2Cgcx2CLY3GD4%2CoHE0GKd5AZI%2CU6jWizrcoaw%2Cmu7OfNdCBE8%2C3MQ47s45dnU%2C44Cbhd1PnWI%2CEdiLL3J89eI%2Cbn91Bs6pE1o%2CZtY3D3hKjXk%2CtRAaFUTA_-A%2CPOXB-v2vNlo%2CkgQncoPA9zE%2CZk_xFgb6Y7Y%2CzNyioBkGDUU%2CzW6ATi1uYhk&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
video_details_p1 <- fromJSON("datasets/video_details_page1.json")
video_details_p2 <- fromJSON("datasets/video_details_page2.json")

vid_id <- c(video_details_p1$items$id, video_details_p2$items$id)

vid_snippet <- bind_rows(video_details_p1$items$snippet,
                         video_details_p2$items$snippet) %>% 
  mutate(id = vid_id) %>% 
  select(id, title, description, publishedAt) %>% 
  tibble()
vid_content_details <- bind_rows(video_details_p1$items$contentDetails,
                                 video_details_p2$items$contentDetails) %>% 
  mutate(id = vid_id) %>% 
  select(id, duration) %>% 
  tibble()

# Function to convert duration to be consistent
convert_duration <- function(duration) {
  # Check if the duration contains only minutes
  if (grepl("^PT\\d+M$", duration)) {
    duration <- paste0(duration, "0S")
  }
  
  # Check if the duration contains only seconds
  if (grepl("^PT\\d+S$", duration)) {
    duration <- paste0("PT0M", gsub("^PT(\\d+)S$", "\\1S", duration))
  }
...
video_details <- left_join(vid_snippet, vid_content_details,
                           by = join_by(id)) %>% 
  mutate(publishedAt = ymd_hms(publishedAt),
         duration = sapply(duration, convert_duration)) %>% 
...
head(video_details) %>% 
  ...
  kable_styling(bootstrap_options = c("condensed",
                ...
                full_width = FALSE)
remove(list = c("video_details_p1", "video_details_p2",
            "vid_id", "vid_snippet", "vid_content_details",
            "convert_duration"))
```

```{r}
video_information <- read_csv(file = "datasets/video_information.csv",
                              show_col_types = FALSE)
video_position <- video_information %>% 
  select(id, position)
video_type <- video_information %>% 
  select(id, talking_head)
```
...
Retrieving audience retention data of videos from subtopic 1.1 Bidang Koordinat. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3DerrB7Ku0HyQ%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
ret20_json_1.1.1 <- fromJSON("datasets/retention_2020_y7zRv0bPAt4.json",
                             flatten = TRUE)
ret20_json_1.1.2 <- fromJSON("datasets/retention_2020_8_HqAYKZZ_I.json",
                             flatten = TRUE)
ret20_json_1.1.3 <- fromJSON("datasets/retention_2020_errB7Ku0HyQ.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.1 <- rbind_pages(
  list(as.data.frame(ret20_json_1.1.1$rows),
       as.data.frame(ret20_json_1.1.2$rows),
       as.data.frame(ret20_json_1.1.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
...
retention_2020_1.1[,c(2, 3, 4)] = apply(retention_2020_1.1[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.1)
```

Retrieving audience retention data of videos from subtopic 1.2 Grafik Persamaan Dua Variabel. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3Dvr7jLRIA7C0%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.2.1 <- fromJSON("datasets/retention_2020_L60C_wcLDHU.json",
                             flatten = TRUE)
ret20_json_1.2.2 <- fromJSON("datasets/retention_2020_Jv4y4Z4zeRc.json",
                             flatten = TRUE)
ret20_json_1.2.3 <- fromJSON("datasets/retention_2020_7Ak8vzFzN_Y.json",
                             flatten = TRUE)
ret20_json_1.2.4 <- fromJSON("datasets/retention_2020_vr7jLRIA7C0.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.2 <- rbind_pages(
  list(as.data.frame(ret20_json_1.2.1$rows),
       as.data.frame(ret20_json_1.2.2$rows),
       as.data.frame(ret20_json_1.2.3$rows),
       as.data.frame(ret20_json_1.2.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.2[,c(2, 3, 4)] = apply(retention_2020_1.2[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.2)
```

Retrieving audience retention data of videos from subtopic 1.3 Garis. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3Dkny1ZHg4tyM%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.3.1 <- fromJSON("datasets/retention_2020_qxsXKYJRgJI.json",
                             flatten = TRUE)
ret20_json_1.3.2 <- fromJSON("datasets/retention_2020_nV01VpbjQLQ.json",
                             flatten = TRUE)
ret20_json_1.3.3 <- fromJSON("datasets/retention_2020_kny1ZHg4tyM.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.3 <- rbind_pages(
  list(as.data.frame(ret20_json_1.3.1$rows),
       as.data.frame(ret20_json_1.3.2$rows),
       as.data.frame(ret20_json_1.3.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.3[,c(2, 3, 4)] = apply(retention_2020_1.3[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.3)
```

Retrieving audience retention data of videos from subtopic 1.4 Persamaan Kuadrat. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3DWZVKP14QVgo%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.4.1 <- fromJSON("datasets/retention_2020_2Wsb2KmeD04.json",
                             flatten = TRUE)
ret20_json_1.4.2 <- fromJSON("datasets/retention_2020_u-qsNS-65pQ.json",
                             flatten = TRUE)
ret20_json_1.4.3 <- fromJSON("datasets/retention_2020_hQTDiy0X4eo.json",
                             flatten = TRUE)
ret20_json_1.4.4 <- fromJSON("datasets/retention_2020_WZVKP14QVgo.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.4 <- rbind_pages(
  list(as.data.frame(ret20_json_1.4.1$rows),
       as.data.frame(ret20_json_1.4.2$rows),
       as.data.frame(ret20_json_1.4.3$rows),
       as.data.frame(ret20_json_1.4.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.4[,c(2, 3, 4)] = apply(retention_2020_1.4[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.4)
```

Retrieving audience retention data of videos from subtopic 1.5 Persamaan-Persamaan Lain. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3DQS8l0H1BGjY%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.5.1 <- fromJSON("datasets/retention_2020_DRr8kXo0Agg.json",
                             flatten = TRUE)
ret20_json_1.5.2 <- fromJSON("datasets/retention_2020_br341Djn3_0.json",
                             flatten = TRUE)
ret20_json_1.5.3 <- fromJSON("datasets/retention_2020_KJKYkGivPi8.json",
                             flatten = TRUE)
ret20_json_1.5.4 <- fromJSON("datasets/retention_2020_QS8l0H1BGjY.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.5 <- rbind_pages(
  list(as.data.frame(ret20_json_1.5.1$rows),
       as.data.frame(ret20_json_1.5.2$rows),
       as.data.frame(ret20_json_1.5.3$rows),
       as.data.frame(ret20_json_1.5.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.5[,c(2, 3, 4)] = apply(retention_2020_1.5[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.5)
```

Retrieving audience retention data of videos from subtopic 1.6 Pertidaksamaan. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3DPcSTCsI6zUk%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.6.1 <- fromJSON("datasets/retention_2020_RF1chK7xiX0.json",
                             flatten = TRUE)
ret20_json_1.6.2 <- fromJSON("datasets/retention_2020_9KVzGWFEufI.json",
                             flatten = TRUE)
ret20_json_1.6.3 <- fromJSON("datasets/retention_2020_PcSTCsI6zUk.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.6 <- rbind_pages(
  list(as.data.frame(ret20_json_1.6.1$rows),
       as.data.frame(ret20_json_1.6.2$rows),
       as.data.frame(ret20_json_1.6.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.6[,c(2, 3, 4)] = apply(retention_2020_1.6[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.6)
```

Retrieving audience retention data of videos from subtopic 1.7 Persamaan dan Pertidaksamaan Nilai Mutlak. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3DqXBG2JV7tms%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.7.1 <- fromJSON("datasets/retention_2020_uDQAWZsh3ns.json",
                             flatten = TRUE)
ret20_json_1.7.2 <- fromJSON("datasets/retention_2020_qXBG2JV7tms.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.7 <- rbind_pages(
  list(as.data.frame(ret20_json_1.7.1$rows),
       as.data.frame(ret20_json_1.7.2$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.7[,c(2, 3, 4)] = apply(retention_2020_1.7[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.7)
```

Retrieving audience retention data of videos from subtopic 1.8 Metode Grafik. The data was retrieved on 8 January 2024.

```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2020-12-31&filters=video%3D%3Dfm6Dv9POaQk%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2020-09-14&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret20_json_1.8.1 <- fromJSON("datasets/retention_2020_v4NPzGXcbaw.json",
                             flatten = TRUE)
ret20_json_1.8.2 <- fromJSON("datasets/retention_2020_fm6Dv9POaQk.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2020_1.8 <- rbind_pages(
  list(as.data.frame(ret20_json_1.8.1$rows),
       as.data.frame(ret20_json_1.8.2$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2020_1.8[,c(2, 3, 4)] = apply(retention_2020_1.8[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2020_1.8)
```
...
retention_2020_1 <- bind_rows(retention_2020_1.1,
                              retention_2020_1.2,
                              retention_2020_1.3,
                              retention_2020_1.4,
                              retention_2020_1.5,
                              ...
                              retention_2020_1.8) %>%
  mutate(year = 2020)
write_csv(retention_2020_1,
          file = "datasets/retention_2020_1.csv")

print(retention_2020_1)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3DerrB7Ku0HyQ%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.1.1 <- fromJSON("datasets/retention_2021_y7zRv0bPAt4.json",
                             flatten = TRUE)
ret21_json_1.1.2 <- fromJSON("datasets/retention_2021_8_HqAYKZZ_I.json",
                             flatten = TRUE)
ret21_json_1.1.3 <- fromJSON("datasets/retention_2021_errB7Ku0HyQ.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.1 <- rbind_pages(
  list(as.data.frame(ret21_json_1.1.1$rows),
       as.data.frame(ret21_json_1.1.2$rows),
       as.data.frame(ret21_json_1.1.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.1[,c(2, 3, 4)] = apply(retention_2021_1.1[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.1)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3Dvr7jLRIA7C0%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.2.1 <- fromJSON("datasets/retention_2021_L60C_wcLDHU.json",
                             flatten = TRUE)
ret21_json_1.2.2 <- fromJSON("datasets/retention_2021_Jv4y4Z4zeRc.json",
                             flatten = TRUE)
ret21_json_1.2.3 <- fromJSON("datasets/retention_2021_7Ak8vzFzN_Y.json",
                             flatten = TRUE)
ret21_json_1.2.4 <- fromJSON("datasets/retention_2021_vr7jLRIA7C0.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.2 <- rbind_pages(
  list(as.data.frame(ret21_json_1.2.1$rows),
       as.data.frame(ret21_json_1.2.2$rows),
       as.data.frame(ret21_json_1.2.3$rows),
       as.data.frame(ret21_json_1.2.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.2[,c(2, 3, 4)] = apply(retention_2021_1.2[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.2)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3Dkny1ZHg4tyM%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.3.1 <- fromJSON("datasets/retention_2021_qxsXKYJRgJI.json",
                             flatten = TRUE)
ret21_json_1.3.2 <- fromJSON("datasets/retention_2021_nV01VpbjQLQ.json",
                             flatten = TRUE)
ret21_json_1.3.3 <- fromJSON("datasets/retention_2021_kny1ZHg4tyM.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.3 <- rbind_pages(
  list(as.data.frame(ret21_json_1.3.1$rows),
       as.data.frame(ret21_json_1.3.2$rows),
       as.data.frame(ret21_json_1.3.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.3[,c(2, 3, 4)] = apply(retention_2021_1.3[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.3)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3DWZVKP14QVgo%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.4.1 <- fromJSON("datasets/retention_2021_2Wsb2KmeD04.json",
                             flatten = TRUE)
ret21_json_1.4.2 <- fromJSON("datasets/retention_2021_u-qsNS-65pQ.json",
                             flatten = TRUE)
ret21_json_1.4.3 <- fromJSON("datasets/retention_2021_hQTDiy0X4eo.json",
                             flatten = TRUE)
ret21_json_1.4.4 <- fromJSON("datasets/retention_2021_WZVKP14QVgo.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.4 <- rbind_pages(
  list(as.data.frame(ret21_json_1.4.1$rows),
       as.data.frame(ret21_json_1.4.2$rows),
       as.data.frame(ret21_json_1.4.3$rows),
       as.data.frame(ret21_json_1.4.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.4[,c(2, 3, 4)] = apply(retention_2021_1.4[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.4)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3DQS8l0H1BGjY%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.5.1 <- fromJSON("datasets/retention_2021_DRr8kXo0Agg.json",
                             flatten = TRUE)
ret21_json_1.5.2 <- fromJSON("datasets/retention_2021_br341Djn3_0.json",
                             flatten = TRUE)
ret21_json_1.5.3 <- fromJSON("datasets/retention_2021_KJKYkGivPi8.json",
                             flatten = TRUE)
ret21_json_1.5.4 <- fromJSON("datasets/retention_2021_QS8l0H1BGjY.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.5 <- rbind_pages(
  list(as.data.frame(ret21_json_1.5.1$rows),
       as.data.frame(ret21_json_1.5.2$rows),
       as.data.frame(ret21_json_1.5.3$rows),
       as.data.frame(ret21_json_1.5.4$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.5[,c(2, 3, 4)] = apply(retention_2021_1.5[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.5)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3DPcSTCsI6zUk%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.6.1 <- fromJSON("datasets/retention_2021_RF1chK7xiX0.json",
                             flatten = TRUE)
ret21_json_1.6.2 <- fromJSON("datasets/retention_2021_9KVzGWFEufI.json",
                             flatten = TRUE)
ret21_json_1.6.3 <- fromJSON("datasets/retention_2021_PcSTCsI6zUk.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.6 <- rbind_pages(
  list(as.data.frame(ret21_json_1.6.1$rows),
       as.data.frame(ret21_json_1.6.2$rows),
       as.data.frame(ret21_json_1.6.3$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.6[,c(2, 3, 4)] = apply(retention_2021_1.6[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.6)
```
...
```         
curl \
  'https://youtubeanalytics.googleapis.com/v2/reports?dimensions=video%2CelapsedVideoTimeRatio&endDate=2021-12-31&filters=video%3D%3DqXBG2JV7tms%3BaudienceType%3D%3DORGANIC&ids=channel%3D%3DMINE&metrics=audienceWatchRatio%2CrelativeRetentionPerformance&startDate=2021-08-22&key=[YOUR_API_KEY]' \
  --header 'Authorization: Bearer [YOUR_ACCESS_TOKEN]' \
  --header 'Accept: application/json' \
  --compressed
```
...
# Creating lists from JSON files
ret21_json_1.7.1 <- fromJSON("datasets/retention_2021_uDQAWZsh3ns.json",
                             flatten = TRUE)
ret21_json_1.7.2 <- fromJSON("datasets/retention_2021_qXBG2JV7tms.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.7 <- rbind_pages(
  list(as.data.frame(ret21_json_1.7.1$rows),
       as.data.frame(ret21_json_1.7.2$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.7[,c(2, 3, 4)] = apply(retention_2021_1.7[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.7)
```
...
# Creating lists from JSON files
ret21_json_1.8.1 <- fromJSON("datasets/retention_2021_v4NPzGXcbaw.json",
                             flatten = TRUE)
ret21_json_1.8.2 <- fromJSON("datasets/retention_2021_fm6Dv9POaQk.json",
                             flatten = TRUE)

# Combining dataframes into a tibble
retention_2021_1.8 <- rbind_pages(
  list(as.data.frame(ret21_json_1.8.1$rows),
       as.data.frame(ret21_json_1.8.2$rows))
) %>% 
  tibble() %>% 
  rename(vid_id = V1,
         elapsed_time_ratio = V2,
...
retention_2021_1.8[,c(2, 3, 4)] = apply(retention_2021_1.8[,c(2, 3, 4)], 2,
                                    function(x)
                                      as.numeric(as.character(x)))

print(retention_2021_1.8)
```
...
retention_2021_1 <- bind_rows(retention_2021_1.1,
                              retention_2021_1.2,
                              retention_2021_1.3,
                              retention_2021_1.4,
                              retention_2021_1.5,
...
write_csv(retention_2021_1,
          file = "datasets/retention_2021_1.csv")

print(retention_2021_1)
```

### Two Year Data

```{r}
retention_1 <- bind_rows(retention_2020_1,
                         retention_2021_1)

write_csv(retention_1,
          file = "datasets/retention_1.csv")
...
```{r}
retention <- bind_rows(retention_1,
                       retention_2,
                       retention_3,
                       retention_4,
                       retention_5) %>% 
...
retention <- retention %>% 
  ...
  mutate(elapsed_time_mins = elapsed_time_ratio*as.numeric(duration)/60) %>% 
  relocate(elapsed_time_mins, .before = elapsed_time_ratio)
